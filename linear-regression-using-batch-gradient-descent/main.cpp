...
void linear_regression(double parameter[],double **input,double output[],int examples,int no_of_feat,double alpha)
{
    ...
    for(i=0;i<no_of_feat;i++)
    {
        ...
        for(int j=0;j<examples;j++)
        {
            ...
            temp[k] = input[j][k];
            cost += (output[j]-dot_product(temp,parameter,no_of_feat))*input[j][i]; // Batch Gradient Descent
        }
...
int main()
{
    ...
    for(i=0;i<10000000;i++)
    {
        ...
        if(i%1==0)
            alpha *= 0.9999999999;      //Decaying Learning Parameter (Depending from Dataset to Dataset)
    ...
    //Calculate the new target
        ...
        double solution = dot_product(x,optimized_parameter,f+1);
