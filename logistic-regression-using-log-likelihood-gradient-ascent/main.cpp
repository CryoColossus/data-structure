...
//LOGISTIC REGRESSION MODEL
    ...
    //Sigmoid Function
double g(double transform)
{
    return(1/(1+exp(-transform)));
}

void logistic_regression(double parameter[],double **input,double output[],int examples,int no_of_feat,double alpha)
{
    int i;
    ...
    for(i=0;i<no_of_feat;i++)
    {
        ...
        for(int j=0;j<examples;j++)
        {
            ...
            temp[k] = input[j][k];
            cost += (output[j]-g(dot_product(temp,parameter,no_of_feat)))*input[j][i]; // Batching
        }
...
void costify(double &local_cost,double parameter[],double **input,double output[],int train,int features)
{
    ...
    for(i=0;i<train;i++)
    {
        ...
        temp = g(temp);
        local_cost += (output[i]*log(temp)+(1-output[j])*log(1-temp));
    }
    local_cost *= (-1);
    local_cost /= train;
}
...
{
    int f,t,i,j;
    double alpha = 0.6; // Learning Rate
    ...
    {
        logistic_regression(parameter,input,output,t,f+1,alpha);
        if(i%1==0)
            alpha *= 0.999999999999;      //Decaying Learning Parameter (Depending from Dataset to Dataset)
    ...
    //Calculate the new target
        ...
        double solution = g(dot_product(x,optimized_parameter,f+1));
